...
export function Welcome() {
  return (
    ...
    >
      <Typography variant="h4" style={{ color: "white", marginTop: "50px" }}>
        Welcome to
      </Typography>
      <img
        src="https://cdn-icons-png.freepik.com/512/3585/3585145.png?ga=GA1.1.1023148225.1707385329&"
        style={{ width: "100px", paddingTop: "20px" }}
      ></img>
      <Typography
        variant="h2"
        style={{ color: "white", marginTop: "20px", fontWeight: "500" }}
      >
        TaskMaster
      </Typography>
      ...
      <Typography
        variant="subtitle1"
        style={{ color: "white", marginTop: "50px", textAlign: "center" }}
      >
        Your trusted ally in productivity, streamlining your to-dos with
        precision, <br />
...
function SignupCard() {
  ...
  const handleSignup = () => {
    axios
      .post(`${BASE_URL}/user/signup`, { username, password })
      .then((response) => {
      ...
      .catch((error) => {
        ...
        if (error.response) {
          ...
          if (error.response.status === 401) {
            ...
            alert(
              error.response.data.issueIn + ": " + error.response.data.problem
            );
          }
        } else {
  ...
  return (
    <div
      id="create-acc"
      style={{ marginRight: "50px", display: "flex", flexDirection: "column" }}
    >
      <Typography variant="h3" style={{ margin: "0 80px 60px" }}>
        Create your account
      </Typography>
      ...
      >
        <Button
          ...
          onClick={() => {
            console.log(
              "inside signup button in signup.tsx, username is: ",
              username,
              "password is: ",
              password
            );
